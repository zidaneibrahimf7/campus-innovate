  ...
  const data: dataProps[] = [
    {
      ...
      free: ['Desain Lower Thried', 'Desain Thumbnail', 'Zoom Pro HD 100 Participants', 'Internet 10 Mbps Kuota 10 GB'],
      adds: ['TV LED 40" 750K', 'Additional Camera 1500K', 'Additional Media Streaming 1000K', 'Gladi Resik Charge 80%', 'Max 8 Jam Kerja 50% Capacity Crew & Alat', 'Setup Alat H-1 Charge 30%'],
      price: 'Rp. 3.600.000',
    },
    {
    ...
    <main>
      <section className="h-[90rem] md:h-[50rem] [perspective:1000px] flex flex-col max-w-[90rem] mx-auto w-full items-start justify-start mt-10 mb-40">
        <Button
          ...
          className="flex gap-2 px-4 py-2 ml-[4rem] mb-4 rounded-md border border-neutral-300 bg-neutral-100 text-primary text-sm hover:-translate-y-1 transform transition duration-200 hover:bg-secondary"
        >
...
export const ContentDetailMeeting = ({ data }: { data: dataProps }) => {
  return (
    <TabServices>
      <h1 className="text-center md:text-left">{data.title}</h1>
        ...
        <article className="flex flex-col gap-2">
          ...
          <ul className="text-[18px] font-normal px-4">
            {data.package.map((res, index) => (
              <li className="list-disc text-black" key={index}>
                {res}{' '}
              </li>
            ))}
          </ul>
